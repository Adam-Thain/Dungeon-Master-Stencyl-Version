	...
	override public function init()
	{
		...
		addWhenDrawingListener(null, function(g:G, x:Float, y:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				...
				if(getActor(3).isMouseOver())
				{
					getActor(3).setAnimation("1");
				}
				if(!(getActor(3).isMouseOver()))
				{
					getActor(3).setAnimation("0");
				}
				if(getActor(4).isMouseOver())
				{
					getActor(4).setAnimation("1");
				}
				if(!(getActor(4).isMouseOver()))
				{
					getActor(4).setAnimation("0");
				}
				if(getActor(5).isMouseOver())
				{
					getActor(5).setAnimation("1");
				}
				if(!(getActor(5).isMouseOver()))
				{
					getActor(5).setAnimation("0");
				}
				if(getActor(6).isMouseOver())
				{
					getActor(6).setAnimation("1");
				}
				if(!(getActor(6).isMouseOver()))
				{
					getActor(6).setAnimation("0");
				}
			}
		});
