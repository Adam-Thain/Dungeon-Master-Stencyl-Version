...
class Design_11_11_GridCameraFollow extends ActorScript
{
	...
	public function new(dummy:Int, actor:Actor, dummy2:Engine)
	{
		...
		nameMap.set("Grid Width", "_GridWidth");
		_GridWidth = 32.0;
		nameMap.set("intendedCameraY", "_intendedCameraY");
		_intendedCameraY = 0.0;
		nameMap.set("Grid Height", "_GridHeight");
		_GridHeight = 32.0;
	...
	override public function init()
	{
		...
		_intendedCameraX = (Math.floor((actor.getXCenter() / _GridWidth)) * _GridWidth);
		_intendedCameraY = (Math.floor((actor.getYCenter() / _GridHeight)) * _GridHeight);
		...
		addWhenUpdatedListener(null, function(elapsedTime:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				...
				engine.moveCamera((_currentCameraX + (_GridWidth / 2)), (_currentCameraY + (_GridHeight / 2)));
			}
		});
